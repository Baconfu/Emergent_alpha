...
void Entity::iterate()
{
    ...
    for(int i=0; i<interactingTiles.length(); i++){
        if(interactingTiles[i]->collision_player()){
            QVector3D clip = collide(interactingTiles[i]->getBox());
            m_geometry->transform(clip);
...
QVector3D Entity::collide(Box box)
{
...
void Entity::updateDisplay()
{
    ...
    m_obj->setZ(int((m_geometry->get110().y() + m_geometry->get110().z()) / Constants::tile_width_pixels));
}
