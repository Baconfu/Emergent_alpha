...
QVector<UnitSpace*> World::getTilesOccupiedPtr(Entity *e)
{
    ...
    for (int i=0; i<e->getSpacesOccupied().length(); i++){
        out.append(getTilePtrFromTilePosition(e->getSpacesOccupied()[i]));
    }
    return out;
}
...
QVector<QVector3D> World::getTilesOccupiedTilePosition(Entity* entity)
{
